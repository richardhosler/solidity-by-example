...
describe("English Auction", () => {
    const [owner, buyer] = new MockProvider().getWallets();
    ...
    before(async () => {
        ...
        EnglishAuction = await deployContract(owner, EnglishAuctionContract, [ERC721.address, 42n, 500n]);
    ...
    it("can start auction", async () => {
        expect(await EnglishAuction.connect(owner).start()).to.emit(EnglishAuction, "Start");
    });
